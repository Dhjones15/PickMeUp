...
async function getData(setListItems,StoreName) {
  firebase
  .database()
  .ref(StoreName+'/')
  .on("value", snapshot => {
...
function addData(itemName,StoreName){
  firebase
    .database()
    .ref(StoreName+'/')
    .push().set({
...
function removeData(key,StoreName) {
  firebase
    .database()
    .ref(StoreName+'/'+key).remove();
}
function removeDataSelection(deleteList,StoreName) {
  firebase
    .database()
    .ref(StoreName+'/').update(
      deleteList
    );
}
...
function setDataStatus(key,itemStatus,StoreName) {
  firebase
    .database()
    .ref(StoreName+'/'+key)
    .update({
...
function setDataName(key,name,StoreName){
  firebase
    .database()
    .ref(StoreName+'/'+key)
    .update({
...
export default function ShoppingList({navigation}) {
  ...
  const StoreName = navigation.state.params
  useEffect(()=>{
    getData(setListItems,StoreName);
  },[]);

  const handleAddItem = () => {
    Keyboard.dismiss();
    if(name===null){return}
    addData(name,StoreName);
    setName(null)
  }
  const handleDeleteItem = (index) => {
    removeData(listItems[index].key,StoreName);
  }
  const changeItemStatus = (index) => {
    setDataStatus(listItems[index].key,listItems[index].status,StoreName);
  }
  ...
  const changeItemName = (index) => {
    if(name===null){return}
    setDataName(listItems[index].key,name,StoreName);
    selectEditItem(null);
    setName(null);
  }
  ...
  const buttonDeleteSelected = () => {
    ...
    listItems.map((item)=>{
      if (item.selected){
        obj= {[item.key]: null};
        removeDataSelection(obj,StoreName);
      }
    })  
